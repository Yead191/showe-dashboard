...
interface ProgrammesState {
  ...
  loadProgramme: (programme: ProgrammeDoc) => void;
...
export const useProgrammesStore = create<ProgrammesState>((set, get) => ({
  programmes: {},
  activeId: null,
  ...
  loadProgramme: (programme) => {
    set((s) => ({
      programmes: { ...s.programmes, [programme.id]: programme },
      activeId: programme.id,
      activePageId: s.activePageId || programme.pages[0]?.id || null,
      selectedBlockId: null,
    }));
  },

  createProgramme: ({ event_id, title }) => {
    const id = newId('prg');
