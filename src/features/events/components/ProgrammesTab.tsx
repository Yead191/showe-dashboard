    ...
    <div className="space-y-6">
      <div className="bg-primary/5 rounded-xl p-4 border border-primary/10">
        ...
        <p className="text-[12.5px] text-ink-muted mt-1 leading-relaxed">
          Link an interactive programme to this event. Audiences will be able to
          access it via the SHOWE app during the performance.
        </p>
      </div>
        ...
        {programmes.length === 0 ? (
          <div className="py-12 text-center border-2 border-dashed border-line rounded-2xl bg-surface-sunken/30">
            ...
            <p className="text-sm text-ink-muted">
              No programmes found in your library.
            </p>
            <p className="text-[12px] text-ink-faint mt-1">
              Create one in the "Programmes" section first.
            </p>
          </div>
        ) : (
                  ...
                  <div
                    className={cn(
                      ...
                      isSelected
                        ...
                        : "bg-surface-sunken text-ink-faint group-hover:bg-surface-sunken/80",
                    )}
                  >
                    <BookOpen size={18} />
                  </div>
      ...
      {selectedId && (
        <div className="p-4 rounded-xl bg-accent/5 border border-accent/10 flex items-start gap-3">
          ...
          <div className="text-[12.5px] text-ink-muted leading-relaxed">
            <strong className="text-ink">Selected for import:</strong> “
            {programmes.find((p) => p.id === selectedId)?.title}” will be linked
            to this event once you save your changes.
          </div>
        </div>
      )}
    </div>
