...
export default function AdminPaymentsPage() {
  ...
  const totals = useMemo(() => {
    const completed = payments.filter(
      (payment) => mapStatus(payment.status) === "succeeded",
    );
    return {
      gross: completed.reduce((sum, payment) => sum + payment.amount, 0),
      fees: completed.reduce(
        (sum, payment) => sum + payment.platform_charge,
        0,
      ),
      count: totalCount,
      failed: payments.filter(
        (payment) => mapStatus(payment.status) === "failed",
      ).length,
    };
  }, [payments, totalCount]);
  ...
  const columns: ColumnsType<ApiPayment> = [
    ...
    },
    {
      ...
      width: 130,
      render: (v: number) => (
        <span className="font-display tabular text-ink-muted">
          {v > 0 ? formatGBP(v) : "—"}
        </span>
      ),
    },
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7 stagger">
        <StatCard
          label="Gross volume"
          value={formatGBP(totals.gross)}
          icon={Banknote}
          accent="primary"
        />
        <StatCard
          label="Platform charge"
          value={formatGBP(totals.fees)}
          icon={TrendingUp}
          accent="amber"
        />
        <StatCard
          ...
          <Tabs
            ...
            items={[
              ...
              },
              {
                ...
                label: tabLabel(
                  "Payments",
                  tab === "Payment" ? totalCount : undefined,
                ),
              },
              {
                key: "Subscription",
                label: tabLabel(
                  "Subscriptions",
                  tab === "Subscription" ? totalCount : undefined,
                ),
              },
            ]}
          />
